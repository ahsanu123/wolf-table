...
function returnMessage(flag: boolean, key: string, ...arg: string[]) {
  let message: string = '';
  if (!flag) {
    const tkey = `validation.${key}` + arg
    message = t(tkey);
  }
  return [flag, message];
}
...
export default class Validator {
  ...
  constructor(type: string, required: boolean, value: string, operator?: string) {
  ...
  equals(other: Validator): boolean {
    let flag: boolean = this.type === other.type
